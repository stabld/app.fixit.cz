...
window.openConversation = async function(requestId, partnerName, partnerSeed, partnerUserId) {
    ...
    // Vymažeme bublinku nepřečtených zpráv pro tento konkrétní chat
    if (!window.STATE.unreadChats) window.STATE.unreadChats = {};
    window.STATE.unreadChats[requestId] = 0;
    
    // Obnovíme levé menu, aby bublinka zmizela
...
window.loadCustomerConversations = async function() {
    ...
    list.innerHTML = reqs.map(r => {
        ...
        // Logika pro zobrazení červené bublinky
        const unreadCount = (window.STATE.unreadChats && window.STATE.unreadChats[r.id]) || 0;
        const unreadBadge = unreadCount > 0 ? `<div class="w-5 h-5 rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center ml-auto shrink-0 shadow-sm">${unreadCount}</div>` : '';
        
        const isActiveClass = window.activeChatId === String(r.id) ? 'bg-white dark:bg-slate-800/50 border-fixit-500' : '';

        return `
        <div id="conv-${r.id}" onclick="window.openConversation(${r.id}, '${safeName}', 'craftsman${r.id}', ${craftIdParam})" class="conv-item px-4 py-3.5 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/60 border-b border-l-2 border-slate-100 dark:border-slate-800/80 transition-all flex items-center gap-3 ${isActiveClass}">
...
window.loadCraftsmanConversations = async function() {
    ...
    list.innerHTML = offers.map(o => {
        ...
        const isActiveClass = window.activeChatId === String(o.request_id) ? 'bg-white dark:bg-slate-800/50 border-fixit-500' : '';

        return `
        <div id="conv-${o.request_id}" onclick="window.openConversation(${o.request_id}, '${safeName}', 'customer${o.request_id}')" class="conv-item px-4 py-3.5 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/60 border-b border-l-2 border-slate-100 dark:border-slate-800/80 transition-all flex items-center gap-3 ${isActiveClass}">
            <div class="relative shrink-0">
...
window.initGlobalNotifications = function() {
    ...
    // Veškeré přijímání notifikací z DB probíhá POUZE ZDE! (Žádné duplicity)
    window.globalNotifSub = window.sb.channel('global-notifs')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, payload => {
            const msg = payload.new;
            
            // Pojistka: Zjistíme, jestli zpráva patří do některé z mých konverzací
            ...
            if (isMyChat && msg.sender_id !== window.APP_USER.id) {
                if (window.activeChatId === String(msg.conversation_id)) {
                    // Chat je OTEVŘENÝ -> Nevyskakuje notifikace, zpráva se rovnou objeví v okně
                    const boxId = window.APP_ROLE === "customer" ? "chat-msgs" : "chat-msgs-c";
                    window.renderMessage(msg, boxId);
                } else {
                    // Chat JE ZAVŘENÝ -> Vyskakuje Toast, puntík na zvonečku a přičítá se červená bublina v chatu
